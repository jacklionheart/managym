...
std::unique_ptr<ActionSpace> DeclareAttackersStep::makeActionSpace(
  ...
  actions.emplace_back(
      new DeclareAttackerAction(attacker, true, active_player, this));
  actions.emplace_back(
      new DeclareAttackerAction(attacker, false, active_player, this));

  return std::make_unique<ActionSpace>(
      active_player, ActionType::DeclareAttacker, std::move(actions));
}
...
std::unique_ptr<ActionSpace> DeclareBlockersStep::makeActionSpace(
    Permanent* blocker) {
  std::vector<std::unique_ptr<Action>> actions;
  ...
  for (Permanent* attacker : combat_phase->attackers) {
    actions.emplace_back(
        new DeclareBlockerAction(blocker, attacker, blocking_player, this));
  }
  actions.emplace_back(
      new DeclareBlockerAction(blocker, nullptr, blocking_player, this));

  return std::make_unique<ActionSpace>(
      blocking_player, ActionType::DeclareBlocker, std::move(actions));
}
