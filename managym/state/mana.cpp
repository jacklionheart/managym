      ...
      switch (c) {
        ...
        default:
          throw std::invalid_argument(
              std::string("Invalid character in mana cost: ") + c);
      }
      i++;
    }
  }
...
bool Mana::canPay(const ManaCost& mana_cost) const {
  ...
  if (total() < mana_cost.manaValue()) {
    spdlog::debug("Not enough total mana (have {}, need {})", total(),
                  mana_cost.manaValue());
    return false;
  }
  ...
  spdlog::debug("For generic cost: need {}, have {} available", generic_needed,
                available_mana);
