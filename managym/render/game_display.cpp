...
GameDisplay::GameDisplay(Game* game)
    : game(game), window(sf::VideoMode(1024, 768), "Magic Game") {
...
GameDisplay::LayoutMetrics GameDisplay::calculateLayout() const {
  ...
  float remaining_height =
      ...
      (metrics.info_section_height + metrics.top_hand_height +
...
void GameDisplay::drawSection(const sf::FloatRect& bounds,
...
void GameDisplay::drawLifeTotals(const sf::FloatRect& bounds) {
  ...
  sf::RectangleShape background(
      sf::Vector2f(bounds.width * 0.2f, bounds.height * 0.8f));
  background.setFillColor(sf::Color(0, 0, 0, 180));
  background.setPosition(bounds.left + bounds.width * 0.05f,
...
void GameDisplay::drawRoundedRect(const sf::FloatRect& bounds,
                                  const sf::Color& fillColor,
                                  const sf::Color& gradientColor,
                                  float cornerRadius, float borderThickness,
...
sf::Text GameDisplay::createText(const std::string& content,
                                 float characterSize, const sf::Color& color) {
  static sf::Font font;
...
void GameDisplay::drawCard(const Card* card, float x, float y, float width,
                           float height, const sf::Color& fillColor,
                           const sf::Color& gradientColor) {
...
void GameDisplay::drawPermanent(const Permanent* permanent, float x, float y,
  ...
  sf::Color fillColor =
      permanent->card->types.isLand() ? theme.LAND_FILL : theme.CREATURE_FILL;
  ...
  if (permanent->tapped) {
    ...
    drawRoundedRect(shadowBounds, sf::Color(0, 0, 0, 50),
                    sf::Color(0, 0, 0, 30), width * theme.CARD_CORNER_RADIUS);
...
void GameDisplay::drawBattlefield(Player* player, const sf::FloatRect& bounds) {
  ...
  drawRoundedRect(
      bounds, theme.SECTION_BACKGROUND,
  ...
  sf::FloatRect land_bounds(bounds.left, bounds.top, bounds.width,
                            land_section_height);
...
void GameDisplay::drawPermanentRow(
    const std::vector<const Permanent*>& permanents,
...
void GameDisplay::render() {
  ...
  drawHand(game->players[1].get(),
           sf::FloatRect(0, layout.getTopHandY(), layout.window_width,
                         layout.top_hand_height));
  drawBattlefield(
      game->players[1].get(),
  ...
  drawHand(game->players[0].get(),
           sf::FloatRect(0, layout.getBottomHandY(), layout.window_width,
