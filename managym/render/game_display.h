...
class GameDisplay {
 ...
 private:
  ...
  static constexpr float CARD_ASPECT_RATIO =
      63.0f / 88.0f;  // Standard MTG card ratio
  static constexpr float RELATIVE_CARD_HEIGHT =
      0.15f;  // Card height relative to window height
  static constexpr float RELATIVE_CARD_SPACING =
      0.005f;  // Spacing relative to window width
  static constexpr float RELATIVE_MARGIN =
      0.02f;  // Margins relative to window size
  ...
  void drawSection(const sf::FloatRect& bounds,
                   const sf::Color& debugColor = sf::Color::Transparent);
  ...
  void drawCard(const Card* card, float x, float y, float width, float height,
                ...
                const sf::Color& gradientColor = sf::Color::White);
  void drawPermanent(const Permanent* permanent, float x, float y, float width,
                     float height);
